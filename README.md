...
| <a name="input_new_vnet"></a> [new\_vnet](#input\_new\_vnet) | When set, a new VNet is created with the given address\_space. An optional name can be provided; if omitted it defaults to '<name>-vnet'. When null (default) the module deploys subnets into the subscription's designated existing VNet resolved from the subscription ID. | <pre>object({<br/>    name          = optional(string)<br/>    address_space = list(string)<br/>  })</pre> | `null` | no |
